...
plt.scatter(data_x, data_y, c='black', s=50, marker="o", linewidth=1, label='Известные данные')
...
data_x = [to_datetime(x, unit='s') for x in data['X']]
data_y = [int(y) for y in data['Y']]
plt.plot(data_x, data_y, color='seagreen', lw=4, label='Интерполированные данные')
...
plt.bar(data_x, data_y, color='grey', lw=1, label='Мероприятия')
...
plt.text(start - timedelta(days=3), 1100, 'Номер', fontsize=10, color='darkblue', ha='left', va='bottom')
plt.text(start - timedelta(days=3), 1150, 'Тип', fontsize=10, color='darkgreen', ha='left', va='bottom')

plt.xlim(start - timedelta(days=5), finish + timedelta(days=5))
plt.ylim(200, 1250)

plt.title('Визуализация данных о количестве подписчиков и проводимых мероприятиях')
plt.xlabel('Дата')
